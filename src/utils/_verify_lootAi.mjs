...
async function callGemini(prompt) {
  const apiKey = (({VITE_GEMINI_API_KEY: 'test-key', VITE_ANTHROPIC_API_KEY: ''}).VITE_GEMINI_API_KEY || '').trim()
...
async function callClaude(prompt) {
  const apiKey = (({VITE_GEMINI_API_KEY: 'test-key', VITE_ANTHROPIC_API_KEY: ''}).VITE_ANTHROPIC_API_KEY || '').trim()
  if (!apiKey) return null
